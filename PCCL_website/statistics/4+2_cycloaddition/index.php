...
<div class="container">
    <h1 class="text-center" style="font-variant: small-caps;">[4+2]-cycloaddition - Statistics</h1>
    <br />
      <div class="row">
...
const data_hac = hac_x.map((k, i) => ({x: k, y: hac_y_R8[i]}));
const data_clogP = clogp_x.map((k, i) => ({x: k, y: clogp_y_R8[i]}));
const data_mw = mw_x.map((k, i) => ({x: k, y: mw_y_R8[i]}));
const data_hba = hba_x.map((k, i) => ({x: k, y: hba_y_R8[i]}));
const data_hbd = hbd_x.map((k, i) => ({x: k, y: hbd_y_R8[i]}));

const data_rotb = rotb_x.map((k, i) => ({x: k, y: rotb_y_R8[i]}));
const data_tpsa = tpsa_x.map((k, i) => ({x: k, y: tpsa_y_R8[i]}));
const data_fsp3 = fsp3_x.map((k, i) => ({x: k, y: fsp3_y_R8[i]}));
const data_qed = qed_x.map((k, i) => ({x: k, y: qed_y_R8[i]}));
